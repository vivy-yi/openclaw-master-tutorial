...
> 本教程深入剖析 OpenClaw 在 Docker 环境下的 Agent 运行机制，涵盖架构原理、容器布局、沙箱隔离、多 Agent 路由、Gateway 集群扩展和完整部署流程。

**更新**：2026-04-21 新增 Gateway 集群与扩展章节
...
```cfg
# haproxy.cfg
frontend openclaw-front
    bind *:18789
    default_backend openclaw-back

backend openclaw-back
    balance roundrobin
    server gw1 gateway-1:18789 check inter 5s fall 2 rise 1
    server gw2 gateway-2:18789 check inter 5s fall 2 rise 1
```
...
│       ▼                ▼                                            │
│  ┌─────────┐    ┌─────────┐    ┌─────────┐                       │
│  │ Agent-A │    │ Agent-B │    │ Agent-C │    ← Agent 池        │
│  │ (Finance)│   │  (Law) │    │ (Ops) │                       │
│  └─────────┘    └─────────┘    └─────────┘                       │
│                                                                  │
│  ┌─────────────────────────────────────────────────────────────┐│
...
### 10.5 Agent 级 Docker 隔离（概念）

#### 当前架构

```
当前：所有 Agent 运行在 Gateway 进程内
┌─────────────────────────────────────┐
│           Gateway 容器                     │
│  ┌───────────────────────────────────┐│
│  │  Agent Loop (同一进程)              ││
...
### 10.6 OpenClaw 与 Docker Agent 的关系

| 场景 | 实现方式 | 说明 |
|------|---------|------|
| **Gateway 部署** | Docker 容器 | 部署整个 OpenClaw Gateway |
| **Gateway 水平扩展** | Docker Compose / K8s | 多个 Gateway 实例 + 负载均衡 |
| **Agent 隔离** | VM2 沙箱（默认） | 进程内逻辑隔离，不是 Docker |
| **高安全隔离** | Docker Agent 容器 | 每个 Agent 独立容器（未来特性）|

### 10.7 Bindings 与 Agent 路由
