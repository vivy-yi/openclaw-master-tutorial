...
> ⚠️ **读者定位**：本文档面向已经完成基础配置、需要深度定制 OpenClaw 的用户。如果你还没有配置过 OpenClaw，请先阅读 [附录J：完整配置参考](./appendix-j/)。
...
> 📖 **前置知识**：了解 Workspace 之前，建议先阅读 [附录J的Agent系统配置](./appendix-j/openclaw-complete-config-reference.md#3-agent-系统配置)，其中 `agents.defaults.workspace` 定义了默认工作区路径。
...
| 文件 | 何时读取 | 何时写入 | 跨Agent共享 |
|------|----------|----------|-------------|
| SOUL.md | 每次启动 | Agent自我更新 | ❌ |
| USER.md | 每次会话 | 用户纠正 | ❌ |
| AGENTS.md | 每次启动 | 架构变更 | ❌ |
| MEMORY.md | 主会话开始 | 重要决策 | ❌ |
| HEARTBEAT.md | 心跳触发 | 每日更新 | ❌ |
| TOOLS.md | 需要时 | 工具配置变更 | ❌ |
| memory/YYYY-MM-DD.md | 参考时 | 每日21:00 | ❌ |
...
为了帮助读者理解内置配置（附录J）和额外配置（附录K）的关系，提供以下对照：

### 配置载体对比

| 维度 | 附录J 内置配置 | 附录K 额外配置 |
|----------|----------------------|----------------------|
...
| 场景 | 推荐文档 |
|------|----------|
| 不知道该用哪个模型 | [附录J - 模型配置](./appendix-j/openclaw-complete-config-reference.md#2-认证和模型提供商) |
| 需要配置 Telegram 频道 | [附录J - 频道集成](./appendix-j/openclaw-complete-config-reference.md#5-频道集成) |
| 希望 Agent 有特定人格 | [附录K - SOUL.md](./openclaw-extra-configuration-guide.md#12-soulmd--agent人格定义) |
| 需要跨 Agent 共享知识 | [附录K - 共享知识库](./openclaw-extra-configuration-guide.md#2-共享知识库) |
| 要让 Agent 学习用户偏好 | [附录K - 自我改进系统](./openclaw-extra-configuration-guide.md#3-自我改进系统) |
| 需要管理多 Agent 协作 | [附录K - 多Agent架构](./openclaw-extra-configuration-guide.md#4-多-agent-架构) |
