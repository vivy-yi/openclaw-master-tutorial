...
### ⚠️ 重要：plugins.allow 白名单配置

如果你的 OpenClaw 配置使用了安全模式（`plugins.mode: "allowlist"`），需要手动将 `lossless-claw` 添加到 `plugins.allow` 列表中：
...
**注意**：
- `plugins.allow` 只接受**插件 ID**（即 `lossless-claw`），不接受命令 token（如 `lossless` 或 `/lcm`）
- 如果不确定自己是否使用了白名单模式，可以运行 `openclaw config get plugins.mode` 检查
