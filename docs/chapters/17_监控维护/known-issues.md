...
## 二、P0/P1 高优先级 Issues（2026-06-13 新增 #92270）
...
### 2.10 #92270 - 会话恢复 stuck bug（2026-06-11 新增）

| 字段 | 内容 |
|------|------|
| **Issue编号** | #92270 |
| **严重程度** | P1 |
| **标题** | Stuck-session recovery never releases — status=aborted with released=0 on every event |
| **状态** | OPEN（无 PR） |
| **创建时间** | 2026-06-11 |
| **影响范围** | 所有渠道的长时间运行 session |
| **标签** | (无官方标签) |

**问题描述**:
会话恢复机制陷入 stuck 状态：
- `status=aborted` 但 `released=0`
- 每个事件都触发错误路径
- Wedge 状态持续约 14 小时
- 即使执行 5 次正确的 target abort 仍无法恢复

**Workaround**:
- 使用 `/reset` 命令（仅对部分情况有效）
- 外部进程重启 Gateway 强制清理 session 锁
- 建议关注后续官方修复

**相关 Issue**: 与 #91529 (OAuth session) 属于不同问题域
...
新增 Issue: #92270, #91540, #91534, #91533, #91532, #91531, #91529, #91525, #91521, #91428, #91526, #91514, #91460, #91433
...
| 2026-06-13 | 新增 #92270 会话恢复 stuck bug（status=aborted released=0，~14h wedge） |

---

🦉 教程大师 | OpenClaw 已知问题追踪 | 2026-06-13
