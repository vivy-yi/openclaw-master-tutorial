...
下图展示了 Browser Use 的完整架构，包括 Agent 控制器、浏览器上下文、LLM 决策引擎、消息管理器等核心组件及其交互关系：

![Browser Use 系统架构图](./browser-use-architecture.png)

### Agent 工作流程图

下图展示了 Browser Use Agent 从接收任务到完成任务的完整执行流程，包括页面状态获取、LLM 推理决策、动作执行、结果验证等关键步骤：

![Browser Use Agent 工作流程图](./browser-use-workflow.png)
...
### 5.1 Agent 可用动作（从源码提取）

**browser-use 的 Agent 使用 Playwright + 19 个预定义动作：**

| 动作类 | 功能 | 参数 |
|--------|------|------|
| `NavigateAction` | 导航到 URL | `url: string` |
| `ClickElementAction` | 点击元素 | `index: int`, `reason?: string` |
| `ClickElementActionIndexOnly` | 按索引点击 | `index: int` |
| `InputTextAction` | 输入文本 | `text: string`, `index?: int` |
| `SearchAction` | 搜索引擎搜索 | `query: string` |
| `SearchPageAction` | 页面内搜索 | `query: string` |
| `FindElementsAction` | 查找元素 | `selector: string` |
| `ExtractAction` | 提取内容 | `goal: string` |
| `ScrollAction` | 滚动页面 | `amount: int`（像素）|
| `SendKeysAction` | 发送按键 | `keys: string` |
| `SwitchTabAction` | 切换标签 | `page_number: int` |
| `CloseTabAction` | 关闭标签 | - |
| `GetDropdownOptionsAction` | 获取下拉选项 | `index: int` |
| `SelectDropdownOptionAction` | 选择下拉选项 | `index: int`, `option: string` |
| `ScreenshotAction` | 截图 | - |
| `SaveAsPdfAction` | 保存 PDF | - |
| `UploadFileAction` | 上传文件 | `path: string`, `index?: int` |
| `StructuredOutputAction` | 结构化输出 | `goal: string`, output_model |
| `DoneAction` | 完成任务 | `output?: string` |
...
- [OpenClaw 内置浏览器](../10_浏览器与系统操作/10.3_browser_automation.md)
