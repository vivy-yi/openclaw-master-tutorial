...
## 18.22 Skill 设计方法论完全指南

### 概述

基于官方 `skill-creator` Skill 的完整方法论，涵盖：

| 内容 | 说明 |
|------|------|
| **Concise is Key** | 上下文窗口是公共资源，只添加 Agent 本身不具备的内容 |
| **Degrees of Freedom** | 高/中/低三种自由度，根据任务特性选择约束程度 |
| **SKILL.md anatomy** | frontmatter + body，description 是唯一触发机制 |
| **Bundled Resources** | scripts/references/assets 三种资源的适用场景 |
| **Progressive Disclosure** | 三级加载系统 + 三种披露模式 |
| **六步创建流程** | 从需求理解到迭代改进的完整流程 |
| **f fireworks-tech-graph 分析** | 用方法论分析实战案例 |

### 核心原则速查

```
高自由度（文本指令）    → 多方案有效、启发式决策
中自由度（伪代码）    → 有首选模式、可接受变化
低自由度（固定脚本）  → 脆弱易错、一致性关键
```

### 三级加载系统

```
Level 1: name + description   → 始终加载 (~100 words)
Level 2: SKILL.md body       → 触发后加载 (<5k words)
Level 3: references/scripts   → 按需加载（无上限）
```

### fireworks-tech-graph 对照分析

| 方法论要求 | fireworks-tech-graph 实践 |
|-----------|--------------------------|
| 触发词完整 | ✅ 中英双语触发词 |
| 渐进式披露 | ✅ 7 种 style 参考独立 |
| scripts/ 辅助 | ✅ 4 个辅助脚本 |
| references/ 分离 | ✅ style-N.md 独立 |

详见：[18-22_Skill设计方法论完全指南.md](18-22_Skill设计方法论完全指南.md)
