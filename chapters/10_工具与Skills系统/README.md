# 05 工具与 Skills

本章节帮助你理解 OpenClaw 的工具系统（Tools）和技能系统（Skills），掌握如何扩展 Agent 能力。

## 🗺️ 三层架构全景

OpenClaw 工具与技能体系遵循**三层同心圆架构**：

```
🟠 Layer 1 Core（~8工具） ─ 原子操作：文件/执行/基础网络
🔵 Layer 2 Advanced（~17工具） ─ 进阶能力：浏览器/记忆/自动化
🟡 Layer 3 Knowledge（53+Skills） ─ 场景封装：办公/开发/安全/生活
```

> 📖 详细图解见 [5.1 工具系统概述](./5.1_tools_overview.md#511-三层架构全景概览)

## 内容导航

| 小节 | 主题 | 预计用时 |
|-----|------|---------|
| [5.1](./5.1_tools_overview.md) | 工具系统概述（★三层架构） | 25分钟 |
| [5.2](./5.2_builtin_tools.md) | 内置工具详解 | 30分钟 |
| [5.3](./5.3_skills_system.md) | Skills 系统 | 30分钟 |
| [5.4](./5.4_skill_market.md) | 技能市场 | 20分钟 |
| [5.5](./5.5_custom_skill.md) | 自定义技能 | 30分钟 |
| [5.6](./5.7_overview.md) | 浏览器自动化 | 40分钟 |
| [5.6_builtin_skills](./5.6_builtin_skills.md) | 内置 Skills 详解 | 30分钟 |
| [5.7](./5.8_clawhub.md) | ClawHub 市场使用 | 20分钟 |
| [5.8](./5.9_skills_scenarios.md) | 常用场景 Skills | 30分钟 |
| [5.9](./5.10_skills_occupation.md) | 职业场景 Skills | 30分钟 |

## 学习目标

完成本章后，你将能够：

- ✅ 理解工具系统的架构
- ✅ 掌握内置工具的使用
- ✅ 使用 Skills 扩展能力
- ✅ 创建自定义技能
- ✅ 掌握内置 Skills 使用方法
- ✅ 使用 ClawHub 安装管理 Skills
- ✅ 构建个人 Skills 工作流
- ✅ 掌握金融投资场景 Skills
- ✅ 掌握运营场景 Skills
- ✅ 掌握炒股投资场景 Skills
- ✅ 构建职业场景工作流

## 核心概念

```
Tools = 原子操作（读、写、执行）
Skills = 能力封装（工作流、提示词、工具组合）
```

## 快速导航

- [上一章：09 日程任务自动化 →](../09_日程任务自动化/)
- [下一章：11 自定义Web界面 →](../11_自定义Web界面/)
- [文档总览 →](../)

## 补充资源

- [OpenClaw Tools 文档](https://docs.openclaw.ai/tools)
- [Skills 市场](https://skills.openclaw.ai)

---

**最后更新**：2026-04-06（由 Tutorial Master Skill 更新章节编号）
